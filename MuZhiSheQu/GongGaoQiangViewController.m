...
-(void)reshData{
    ...
    [anle getNoticeListwallWithDic:dic Block:^(id models, NSString *code, NSString *msg) {
        NSLog(@"getNoticeListwallWithDic==%@",models);
...
//    [self.view addSubview:_bgCon];
    ...
    [_bv addSubview:fa];
    UIButton *hiddleKeyBtn = [[UIButton alloc]initWithFrame:CGRectMake(self.view.width-120*self.scale, _mesay.bottom+10*self.scale, 50*self.scale, 30*self.scale)];
    [hiddleKeyBtn setBackgroundImage:[UIImage ImageForColor:[UIColor colorWithRed:224/255.0 green:234/255.0 blue:238/255.0 alpha:0]]forState:UIControlStateNormal];
    hiddleKeyBtn.layer.cornerRadius=5;
    hiddleKeyBtn.layer.borderWidth=.5;
    hiddleKeyBtn.layer.borderColor=blackLineColore.CGColor;
    hiddleKeyBtn.titleLabel.font=DefaultFont(self.scale);
    [hiddleKeyBtn setTitle:@"取消" forState:UIControlStateNormal];
    [hiddleKeyBtn addTarget:self action:@selector(hiddleKeyboard) forControlEvents:UIControlEventTouchUpInside];
    [hiddleKeyBtn setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
    [_bv addSubview:hiddleKeyBtn];
}
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    NSLog(@"headimg==%d==%@",indexPath.row,_data[indexPath.row][@"logo"]);
