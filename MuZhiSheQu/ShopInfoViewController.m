...
- (void)viewDidLoad {
    [super viewDidLoad];
   // NSLog(@"indexNumber====%d========",_index);
   // NSLog(@"param=-=-0000==%@",self.prod_id);
    if (_isPush){
...
-(void)ReshData{
    ...
    NSDictionary *dic = @{@"shop_id":self.shop_id,@"prod_id":self.prod_id};
    if ([Stockpile sharedStockpile].isLogin) {
        dic = @{@"prod_id":self.prod_id};
    }
    AnalyzeObject *analyze=[[AnalyzeObject alloc]init];
    //  getprodDetailwithDic
    [analyze getprodDetailPushwithDic:dic WithBlock:^(id models, NSString *code, NSString *msg) {
        NSLog(@"getprodDetailPushwithDic======%@======",models);
        ...
        if ([code isEqualToString:@"0"]) {
            self.indexs=0;
            self.yes=NO;
            if ([[NSString stringWithFormat:@"%@",models[@"shop_info"][@"is_open_chat"]] isEqualToString:@"2"]) {
                self.isopen=NO;
            }else{
                self.isopen=YES;
            }
            self.price =models[@"price"];
            //                self.numb = 0;
            self.shop_name=models[@"shop_name"];
            self.shop_user_id=models[@"shop_user_id"];
            //                self.indexNumber = 0;
            self.shop_id = [models objectForKey:@"shop_id"];
            self.prod_id = [models objectForKey:@"id"];
            self.xiaoliang= models[@"sales"];
            self.shoucang= models[@"collect_time"];
            self.tel = models[@"hotline"];
            
            _detailData=models;
            if ([models[@"is_collect"] isEqualToString:@"1"]){
...
//    }
    ...
    //if ([Stockpile sharedStockpile].isLogin) {
        _index=[[DataBase sharedDataBase] getProdCountByProdId:self.prod_id];
    //}
    [self centerCont];
    _shopCarLa.attributedText = [self jiSuanBenDianXiaoFei];
}
...
//            NSString *url=@"";
            ...
            // NSString *smallImgUrl=[NSString stringWithFormat:@"%@/%@",path,[imagename stringByReplacingOccurrencesOfString:@"." withString:@"_thumb640."]];
              NSString *smallImgUrl=[NSString stringWithFormat:@"%@/%@",path,imagename];
...
-(void)centerCont{
    ...
    self.name.text = [_detailData objectForKey:@"prod_name"];
...
//        jianBtn.hidden=YES;
    //    }else{
    NSString* prodID=_detailData[@"id"];
...
//    if ([_detailData[@"inventory"] isEqualToString:@"0"]) {
//        jiaBtn.hidden=YES;
//    }else{
//        jiaBtn.hidden=NO;
//    }
...
//    }
   ...
   // if (_data.count>0) {
        NSString * priceString =[NSString stringWithFormat:@"￥%@/%@",[_detailData objectForKey:@"price"],_detailData[@"unit"]];
        NSString * firstString = [NSString stringWithFormat:@"￥%@",[_detailData objectForKey:@"price"]];
        NSString * secondString = [NSString stringWithFormat:@"/%@",[_detailData objectForKey:@"unit"]];
...
//    _name.width=self.view.width-_priceLa.left;
    ...
    if (![_detailData[@"origin_price"] isEqualToString:@""]) {
        oldPrice.text=[NSString stringWithFormat:@"￥%@",_detailData[@"origin_price"]];
    }else{
    ...
    float yuan = [[NSString stringWithFormat:@"%@",_detailData[@"origin_price"]] floatValue];
    float xian = [[NSString stringWithFormat:@"%@",[_detailData objectForKey:@"price"]] floatValue];
...
//    }
    ...
    self.info.text = [NSString stringWithFormat:@"%@",[[_detailData objectForKey:@"description"] trimString]];
    [vi addSubview:_info];
    if (!self.info.text) {
        self.info.text=@"";
    }
...
//    }
    ...
    NSInteger isOffLine=[[NSString stringWithFormat:@"%@",self.param[@"is_off_online"]] integerValue];
    if(isOffLine==1){
        ...
        [alertView show];
        return;
    }else if(![AppUtil isDoBusiness:self.param]){
        UIAlertView *alertView = [[UIAlertView alloc] initWithTitle:@"歇业中"
                                                            message:[NSString stringWithFormat:@"%@",self.param[@"onlinemark"]]
                                                           delegate:nil
                                                  cancelButtonTitle:@"确定"
                                                  otherButtonTitles:nil];
        [alertView show];
    ...
    NSMutableDictionary* par=[self.param mutableCopy];
    [par setObject:self.prod_id forKey:@"prod_id"];
    if (_orshoucang) {//
        if (btn.tag==456) {
            _numb--;
            _index--;
        ...
        int index=[self.appdelegate.shopDictionary[@([self.prod_id intValue])] intValue];

        NSString * value = [[NSUserDefaults standardUserDefaults]objectForKey:@"GouWuCheShuLiang"];
        int cartNum=[value intValue];
        if([_type isEqualToString:@"2"]){
            NSLog(@"param=-=-1111=%d==%@",index,self.appdelegate.shopDictionary);
             index++;
            [[DataBase sharedDataBase] updateCart:par withType:1];
            [self.appdelegate.shopDictionary setObject:[NSString stringWithFormat:@"%d",index] forKey:@([self.prod_id intValue])];
            NSLog(@"param=-=-2222=%d==%@",index,self.appdelegate.shopDictionary);
       ...
       // [self shangJiaXiangQing];
        ...
        //NSLog(@"indexNumber==%d    index==",self.index);
        int index=[self.appdelegate.shopDictionary[@([self.prod_id intValue])] intValue];
        if (btn.tag==456) {
            _numb--;
            _index--;
        ...
        if (_delegate && [_delegate respondsToSelector:@selector(addNumberindex:number:jiaAndJian:)]) {
            if([_type isEqualToString:@"1"])
                [_delegate addNumberindex:self.indexNumber number:_index jiaAndJian:NO];
            else if([_type isEqualToString:@"2"])
                [_delegate addNumberindex:self.indexNumber number:_index jiaAndJian:YES];
        }
    }
//    if (btn.tag==456) {
...
-(void)bottomVi{
    ...
    [_numberImg setTitle:[NSString stringWithFormat:@"%d",totalNumber] forState:UIControlStateNormal];
...
//        [self.navigationController pushViewController: rcd animated:YES];
        ...
        NSDictionary *dic = @{@"user_id":[self getuserid],@"collect_type":@"1",@"collect_id":_detailData[@"id"]};
...
//        info.isopen=YES;
    ...
    info.tel=_tel;
    info.shop_id=self.shop_id;
    info.ID=self.shop_id;
    info.titlete=_shop_name;
    info.shop_user_id=_shop_user_id;
    info.issleep=_issleep;
    info.manduoshaofree=[_detailData objectForKey:@"free_delivery_amount"];
    NSString *ID = [_detailData objectForKey:@"id"];
...
//        
//    }
    _carPrice = [NSString stringWithFormat:@"本店消费￥%.2f元",[self jiSuanAmount]];
