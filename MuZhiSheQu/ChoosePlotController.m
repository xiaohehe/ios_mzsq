...
- (void)viewDidLoad {
    ...
    dangerAreaHeight=self.isIphoneX?34:0;
  ...
  @{NSFontAttributeName:[UIFont systemFontOfSize:18],
   ...
   // searchTf.placeholder=@"请输入小区名称";
    ...
    plotTv=[[UITableView alloc] initWithFrame:CGRectMake(0, nearbyView.bottom, self.view.frame.size.width,self.view.frame.size.height-nearbyView.bottom-([self getStartHeight]+44)-44-dangerAreaHeight )];
    ...
    openBtn=[[UIButton alloc] initWithFrame:CGRectMake(0, plotTv.bottom, self.view.frame.size.width, 44+dangerAreaHeight)];
    openBtn.backgroundColor=[UIColor colorWithRed:0.498 green:0.498 blue:0.498 alpha:1.00];
    [openBtn setTitle:@"如果您的小区还没有开通拇指便利，请访问www.mzsq.com"  forState:UIControlStateNormal];
    openBtn.titleLabel.font = [UIFont systemFontOfSize:13];//title字体大小
    openBtn.titleLabel.textAlignment = NSTextAlignmentCenter;//设置title的字体居中
    [openBtn setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];//设置title在一般情况下为白色字体
    [openBtn addTarget:self action:@selector(skipOpen) forControlEvents:UIControlEventTouchUpInside];
    [self.view addSubview:openBtn];
}

- (void)skipOpen{
    NSLog(@"====123456");
    [[UIApplication sharedApplication] openURL:[NSURL URLWithString:@"http://www.mzsq.com"]];
}
...
-(void) skipMain{
    ...
    [[NSUserDefaults standardUserDefaults]setObject:currentPlot.shopid forKey:@"shopid"];
...
-(void) requestNearlyPlot{
    ...
    [anle getNearbyCommunity:dic Block:^(id models, NSString *code, NSString *msg) {
        ...
        for(NSDictionary* obj in arr){
            ...
            plot.shopid=[obj objectForKey:@"shopid"];
...
//        plotTv.userInteractionEnabled=NO;
        ...
        [[NSUserDefaults standardUserDefaults]setObject:plot.shopid forKey:@"shopid"];
        [[NSUserDefaults standardUserDefaults]setBool:YES forKey:@"changeComm"];
        [[NSUserDefaults standardUserDefaults]setBool:YES forKey:@"changeCommShang"];
        [[NSUserDefaults standardUserDefaults]synchronize];
        NSString *tag =[NSString stringWithFormat:@"%@",[[NSUserDefaults standardUserDefaults] objectForKey:@"commid"]];
    ...
    [dic setObject:plot.province forKey:@"provinceid"];
    [dic setObject:plot.city forKey:@"cityid"];
    [dic setObject:plot.district forKey:@"districtid"];
